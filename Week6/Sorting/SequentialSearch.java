...
public class SequentialSearch {
    ...
    public static void sequentialSearch(int[] parameterOne, int parameterTwo) {
        int index = -1;
        //Searching each index of the array until it reaches the last index
        for (int i = 0; i < parameterOne.length; i++) {
            if (parameterOne[i] == parameterTwo) {
                ...
                //the for loop is terminared
                index = i;
                break;
            }
        }
        if (index == -1) {
            System.out.println("Your target integer does not exits in the array");
        } else {
